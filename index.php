...
if ($_POST['like']) {
        ...
        $timelike = date("Y-m-d", time());
	$mysql->query("UPDATE ".$tb_prefix."film SET film_like = film_like+1, time_like =$timelike WHERE film_id = '$film_id'");
	$r = $mysql->fetch_array($mysql->query("SELECT film_like, film_dislike FROM ".$tb_prefix."film WHERE film_id = '$film_id'"));
  	echo $r['film_like'].' Like';
	exit();
}
if ($_POST['showfilm']) {
   if ($num == 1) $type = 'new';
...
if($_POST['nextmovie']) {
	...
	if($next) $link_next	=	$web_link.'/xem-phim/'.replace($next['episode_id']).'-'.replace($phihung['film_name_ascii']).'-tap-'.replace($next['episode_name']).'.html';
...
#######################################
	...
	else if  (($value[1]=='user') || ($value[1]=='news') || ($value[1]=='zchannel'))
