...
//Star sitemap
	$rss .= "<url>\r\n";
	$rss .= "<loc>http://www.face.cuatoi.net/</loc>\r\n";
...
{
	$film_link ='http://www.face.cuatoi.net/the-loai/'.replace($rs['cat_name_ascii']).'.html';
...
while ($rs = mysql_fetch_array($select))
{
	$film_link = 'http://www.face.cuatoi.net/the-loai/'.replace($rs['country_name_ascii']).'.html';
	$time	= date('c',$rs["film_date"]);
	$rss .= "   <url>\r\n";
	$rss .= "		<loc>".$film_link."</loc>\r\n";
	$rss .= "   		<lastmod>".date('c',time())."</lastmod>\r\n";
...
while ($rs = mysql_fetch_array($select))
{
	$film_link = 'http://www.face.cuatoi.net/dien-vien/phim-'.replace($rs['actor_name_kd']).'.html';
	$time	= date('c',$rs["film_date"]);
	$rss .= "   <url>\r\n";
	$rss .= "		<loc>".$film_link."</loc>\r\n";
	$rss .= "   		<lastmod>".date('c',time())."</lastmod>\r\n";
...
{
	$rss .= "   <url>\r\n";
	$rss .= "		<loc>http://www.face.cuatoi.net/thong-tin-phim/".$row['film_id']."/".clean_feed(replace($row['film_name_ascii'])).".html</loc>\r\n";
	$rss .= "   		<lastmod>".date('c',time())."</lastmod>\r\n";
	...
	$rss .= "</urlset>\r\n";
echo $rss;
file_put_contents('sitemap-animet47_net.xml',$rss);
}
}
